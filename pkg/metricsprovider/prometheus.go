...
func init() {
	var ok bool
	promAddress, ok = os.LookupEnv(promHostKey)
	if !ok {
		promAddress = DefaultPromAddress
	}
	promToken, ok = os.LookupEnv(promTokenKey)
}
...
func NewPromClient(opts MetricsProviderOpts) (watcher.FetcherClient, error) {
